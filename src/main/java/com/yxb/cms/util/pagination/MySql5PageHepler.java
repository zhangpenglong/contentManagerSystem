...
public class MySql5PageHepler {
    ...
    public static String getCountString(String querySelect) {
        ...
        if (select.toLowerCase().indexOf("select distinct") != -1
            ...
            return new StringBuffer(querySelect.length()).append("select count(1) count from (")
                    .append(querySelect.substring(0, orderIndex)).append(" ) total").toString();
        } else {
    ...
    public static void main(String[] args) {
    	//String querySelect="SELECT    t.id   FROM da_order  t   LEFT JOIN da_invoice i ON t.id = i.order_id";//count 错误，必须有order by 作为分页标示
    	String querySelect="SELECT  t.id FROM da_order t LEFT JOIN da_invoice i ON t.id = i.order_id where id in(1,2,3) order by t.order_date desc";//会自动进行正确的分页
    	//String querySelect="SELECT distinct id FROM da_order t LEFT JOIN da_invoice i ON t.id = i.order_id order by t.order_date desc";//对于分组语句，会绕道而行
    	String limitString = MySql5PageHepler.getLimitString(querySelect, 3, 5);
    	System.out.println(limitString);
    	System.out.println(MySql5PageHepler.getCountString(querySelect));
	}
}